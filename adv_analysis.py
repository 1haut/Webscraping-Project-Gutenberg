...
def dict_lookup(word):
    ...
    if resp.status_code == 200:
        ...
        f = open("cached_books/temp_cache2.json", "w")
        f.write(text)
        f.close()
...
"""
    ...
    #     total += sen.sentiment.polarity

    # polar = total / len(blob.sentences)
    # print(f"{polar:.2f}")

    shortest_sentence = longest_sentence = blob.sentences[0]
    wordiest_sentence = (blob.sentences[0], len(tokenize(str(new_sen))))
    
    for new_sen in blob.sentences[1:]:
        words = tokenize(str(new_sen))
...
def word_class_tag():
    ...
    print(f"{word_class["NOUN"] / len(tokens_tag) * 100:.2f}% NOUN")
